...
class Checklistexport extends CI_Controller
{
	...
	private function generateadvchecklist($advlist)
	{
	    ...
	    $fontstyle->setName("Ariel")->setSize(10);
	    //Cell A4 -- A lot of code to make a single word green
	    $text = new PHPExcel_RichText();
	    $run1 = $text->createTextRun("The term and year of current (ongoing) courses have been highlighted in ")->setFont($fontstyle);
	    $run2 = $text->createTextRun("green")->setFont($fontstyle);
	    $run2->getFont()->setColor(new PHPExcel_Style_Color(PHPExcel_Style_Color::COLOR_GREEN));
	    $run3 = $text->createTextRun(" on the check sheet.")->setFont($fontStyle);
	    ...
	    $run2->getFont()->setColor(new PHPExcel_Style_Color(PHPExcel_Style_Color::COLOR_YELLOW));
	    ...
	    $advlist->getRowDimension(3)->setRowHeight(12*2+1);
	    $advlist->getRowDimension(4)->setRowHeight(12*2+1);
	    $advlist->getRowDimension(5)->setRowHeight(12*2+1);
	    $advlist->getRowDimension(6)->setRowHeight(12*2+1);
	    $advlist->getRowDimension(7)->setRowHeight(12*2+1);
	    $advlist->getRowDimension(8)->setRowHeight(12*2+1);
	    $advlist->getRowDimension(9)->setRowHeight(12*2+1);
	    $advlist->getRowDimension(10)->setRowHeight(12*2+1);
	    $advlist->getRowDimension(11)->setRowHeight(12*2+1);
	    $advlist->getRowDimension(12)->setRowHeight(12*3+1);
	}
